...
export default function Dashboard() {
  ...
  const save = async (event) => {
    ...
    if (formData.studentCount !== '' && (!Number.isFinite(Number(formData.studentCount)) || Number(formData.studentCount) < 0)) validationErrors.studentCount = 'Enter a valid student count.';
    if (!isHttpUrl(formData.locationLink)) validationErrors.locationLink = 'Enter a valid http or https map link.';
    ...
    if (workflowStatus(formData) === 'completed') {
      ...
      if (outcomes.attendance !== '' && (!Number.isFinite(Number(outcomes.attendance)) || Number(outcomes.attendance) < 0)) validationErrors.attendance = 'Enter a valid attendance count.';
      outcomes.photoLinks.forEach((link, index) => {
        if (!isHttpUrl(link)) validationErrors[`photo-${index}`] = 'Enter a valid http or https link.';
      });
    }
